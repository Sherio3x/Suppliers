...
import 'package:supplier_invoice_app/notifiers/supplier_account_summary_notifier.dart';
...
class _SupplierAccountSummaryScreenState extends State<SupplierAccountSummaryScreen> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      Provider.of<SupplierAccountSummaryNotifier>(context, listen: false)
          .fetchSummary(widget.supplier.id!); 
    });
  }
  ...
  Widget build(BuildContext context) {
    return ChangeNotifierProvider(
      create: (_) => SupplierAccountSummaryNotifier(),
      child: Scaffold(
        appBar: AppBar(
        ...
        body: Padding(
          ...
          child: Column(
            ...
            children: [
              Consumer<SupplierAccountSummaryNotifier>(
                builder: (context, notifier, child) {
                  if (notifier.summaryState == SummaryState.loading) {
                    return const Center(child: CircularProgressIndicator());
                  }
                  if (notifier.summaryState == SummaryState.error) {
                    return Center(child: Text('خطأ: ${notifier.errorMessage}'));
                  }
                  if (notifier.summaryData == null || notifier.summaryData!.isEmpty) {
                    return const Center(child: Text('لا توجد بيانات ملخص لهذا المورد.'));
                  }

                  final summary = notifier.summaryData!;
                  return Card(
              ...
              const SizedBox(height: 8),
              Expanded(
                child: Consumer<SupplierAccountSummaryNotifier>(
                  builder: (context, notifier, child) {
                    return StreamBuilder<List<dynamic>>(
                      stream: notifier.getMovements(widget.supplier.id!),
